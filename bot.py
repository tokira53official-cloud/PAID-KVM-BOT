...
async def delete_vps(ctx, user: discord.Member, vps_number: int, *, reason: str = "No reason"):
    ...
    # 4️⃣ Success embed
    embed = create_success_embed("🌟 PrimeCloud - VPS Deleted Successfully")
...
async def user_info(ctx, user: discord.Member):
    ...
    embed.set_footer(text="PrimeCloud • User Resource Dashboard")
    embed.timestamp = ctx.message.created_at
...
async def server_stats(ctx):
    ...
    embed.set_footer(text="PrimeCloud • Real-Time Monitoring")
    embed.timestamp = ctx.message.created_at
