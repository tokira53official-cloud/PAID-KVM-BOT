...
BOT_NAME = os.getenv('BOT_NAME', 'NotRyxenYT')
...
def kvm_translate(c):
    ...
    if m:
        n,mb=m.groups(); return f"virsh setmaxmem {shlex.quote(n)} {mb}M --config; virsh setmem {shlex.quote(n)} {mb}M --config"
...
async def delete_vps(ctx, user: discord.Member, vps_number: int, *, reason: str = "No reason"):
    ...
    # 4️⃣ Success embed
    embed = create_success_embed("🌟 NotRyxenYT - VPS Deleted Successfully")
...
async def user_info(ctx, user: discord.Member):
    ...
    embed.set_footer(text="NotRyxenYT • User Resource Dashboard")
    embed.timestamp = ctx.message.created_at
...
async def server_stats(ctx):
    ...
    embed.set_footer(text="NotRyxenYT • Real-Time Monitoring")
    embed.timestamp = ctx.message.created_at
